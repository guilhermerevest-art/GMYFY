...
serve(async (req) => {
  ...
  try {
    ...
    if (!qrSecret) {
      return new Response(JSON.stringify({ message: 'Configuração inválida' }), {
    ...
    try {
      ...
      return new Response(JSON.stringify({ message: 'QR Code inválido ou expirado' }), {
        status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    if (payload.academiaId !== academiaId) {
      return new Response(JSON.stringify({ message: 'QR Code não pertence a esta academia' }), {
    ...
    if ((count ?? 0) > 0) {
      return new Response(JSON.stringify({ message: 'Você já fez check-in hoje nesta academia' }), {
    ...
    return new Response(JSON.stringify({ pontosGanhos, descricoes }), {
